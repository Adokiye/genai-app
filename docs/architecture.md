...
- **apps/api** — a NestJS service that fabricates completions through a mock LLM, evaluates them with deterministic heuristics, and persists the entire experiment log in `apps/api/data/experiments.json` for versioned audits.
...
3. `ExperimentsService` iterates over every parameter combination, asks `LlmService` for text, feeds each response into `MetricsService`, and attaches a human readable analysis string.
...
- **Deterministic mock LLM**: the API never calls external services, guaranteeing reproducible experiments that can run in CI and demo environments.
